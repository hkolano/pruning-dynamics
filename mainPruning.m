...
for i = 1:length(t_vec)
    ...
    elseif top_hit == 1 && bot_hit == 1
        [forces, wrenches] = get_forces_attached(p, x_state);
    else
        forces = zeros(1,8);
        wrenches = zeros(1,6);
    end
...
%% FUNCTIONS
function [Forces, wrench] = get_forces_attached(p, x_state)
    % Get restoring Forces of branch
    [F_Kx, F_Ky, ~, ~] = getRestoringForces(p, x_state);
    
    % Get center points and angles of the normal contact w/both blades
